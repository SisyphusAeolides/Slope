...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum NodeKind { File, Directory, Device, Hologram, Symlink }
...
pub fn close(handle: AkashicHandle) -> Result<(), FsError> {
    ...
    unsafe { syscall(SYS_ACLOSE, args) }.map(|_| ()).map_err(Into::into)
}

pub fn read(handle: AkashicHandle, buf: &mut [u8]) -> Result<usize, FsError> {
    let args = [handle.0 as usize, buf.as_mut_ptr() as usize, buf.len(), 0, 0, 0];
...
pub fn seek(handle: AkashicHandle, offset: i64, whence: u32) -> Result<u64, FsError> {
    ...
    unsafe { syscall(SYS_ASEEK, args) }.map(|v| v as u64).map_err(Into::into)
}
...
pub fn mkdir(path: &[u8]) -> Result<(), FsError> {
    ...
    unsafe { syscall(SYS_AMKDIR, args) }.map(|_| ()).map_err(Into::into)
}

pub fn unlink(path: &[u8]) -> Result<(), FsError> {
    let args = [path.as_ptr() as usize, path.len(), 0, 0, 0, 0];
    unsafe { syscall(SYS_AUNLINK, args) }.map(|_| ()).map_err(Into::into)
}

pub fn rename(from: &[u8], to: &[u8]) -> Result<(), FsError> {
    let args = [
        from.as_ptr() as usize, from.len(),
        to.as_ptr()   as usize, to.len(),
        0, 0,
    ];
    unsafe { syscall(SYS_ARENAME, args) }.map(|_| ()).map_err(Into::into)
}
...
impl AkashicFile {
    ...
    pub fn write_all(&self, mut buf: &[u8]) -> Result<(), FsError> {
        while !buf.is_empty() {
            ...
            if n == 0 { return Err(FsError::Io(SyscallError(-5))); }
            buf = &buf[n..];
        }
        Ok(())
    }
    ...
    pub const fn handle(&self) -> AkashicHandle { self.handle }
}

impl Drop for AkashicFile {
    fn drop(&mut self) { let _ = close(self.handle); }
}
...
impl RawStat {
    const fn zeroed() -> Self {
        Self { size_bytes: 0, created_ticks: 0, modified_ticks: 0,
               flags: 0, kind: 0, _pad: [0; 3] }
    }
}
...
impl RawDirent {
    const fn zeroed() -> Self {
        Self { name: [0; DIRENT_NAME_MAX], name_len: 0, kind: 0, _pad: [0; 6] }
    }
}
