...
pub enum ThermalZone { Cold = 0, Warm = 1, Hot = 2, Critical = 3 }
...
impl ThermalZone {
    ...
    pub const fn is_throttled(self) -> bool { self as u8 >= 2 }
}

// ─── THROTTLE HINT ─────────────────────────────────────────────────────────

#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum ThrottleHint { Full = 0, Half = 1, Quarter = 2, Minimal = 3 }
...
impl<'page> ThermalGuard<'page> {
    ...
    pub fn current_hint(&self) -> ThrottleHint { self.hint }
...
impl<'page> ThermalPolicy<'page> {
    ...
    pub fn current_zone(&self) -> ThermalZone { self.last_zone }
}
...
mod tests {
    ...
    fn throttled_batch_size_scales_correctly() {
        ...
        page.throttle_hint.store(3, core::sync::atomic::Ordering::Relaxed);
