...
impl SlabPage {
    fn new(base: *mut u8, object_size: usize) -> Self {
        ...
        Self {
            base, free_mask: mask, object_size,
            slot_count, free_count: slot_count.min(u16::MAX as usize) as u16,
        }
    }
    ...
    fn alloc_slot(&mut self) -> Option<*mut u8> {
        ...
        for wi in 0..word_count.min(BITMASK_WORDS) {
            if self.free_mask[wi] == 0 { continue; }
            let bit = self.free_mask[wi].trailing_zeros() as usize;
            let slot = wi * 64 + bit;
            if slot >= self.slot_count { continue; }
    ...
    fn dealloc_slot(&mut self, ptr: *mut u8) -> bool {
        ...
        let wi  = slot / 64;
        let bit = slot % 64;
        if self.free_mask[wi] & (1u64 << bit) != 0 { return false; } // double-free
        self.free_mask[wi] |= 1u64 << bit;
        self.free_count += 1;
        true
    }

    const fn _has_free(&self) -> bool { self.free_count > 0 }
}
...
impl SlabClass {
    ...
    fn alloc(&mut self) -> Option<*mut u8> {
        ...
        for p in self.pages[..self.page_count].iter_mut().flatten() {
            if let Some(ptr) = p.alloc_slot() { return Some(ptr); }
        }
    ...
    fn grow(&mut self) -> Option<()> {
        if self.page_count >= MAX_SLAB_PAGES { return None; }
    ...
    fn dealloc(&mut self, ptr: *mut u8) -> bool {
        for p in self.pages[..self.page_count].iter_mut().flatten() {
            if p.dealloc_slot(ptr) { return true; }
        }
        false
    }
}
...
impl GlobalSlabHeap {
    ...
    pub fn init(&self) { self.ready.store(true, Ordering::Release); }
    ...
    pub fn stats(&self) -> HeapStats {
        ...
        }).unwrap_or(HeapStats { alloc_count: 0, dealloc_count: 0, oom_count: 0 })
    }
}
...
unsafe impl GlobalAlloc for GlobalSlabHeap {
    unsafe fn alloc(&self, layout: Layout) -> *mut u8 {
        if !self.ready.load(Ordering::Acquire) { return core::ptr::null_mut(); }
        let size = layout.size().max(layout.align());
        let ci   = match class_for(size) { Some(c) => c, None => return core::ptr::null_mut() };
        let pair = unsafe {
            crate::sync::entanglement::EntangledPair::from_mapping(&self.cell, 0)
        };
        pair.mutate_bounded(64, |h| {
            let ptr = h.classes[ci].alloc().unwrap_or(core::ptr::null_mut());
            if ptr.is_null() { h.oom_count += 1; } else { h.alloc_count += 1; }
            ptr
    ...
    unsafe fn dealloc(&self, ptr: *mut u8, layout: Layout) {
        ...
        let size = layout.size().max(layout.align());
        let ci   = match class_for(size) { Some(c) => c, None => return };
        let pair = unsafe {
            crate::sync::entanglement::EntangledPair::from_mapping(&self.cell, 0)
        };
        let _ = pair.mutate_bounded(64, |h| {
            if h.classes[ci].dealloc(ptr) { h.dealloc_count += 1; }
        });
    }
}
...
impl SlabToken {
    ...
    pub const fn class(&self) -> u8  { (self.0 >> 24) as u8 }
    pub const fn page(&self)  -> u8  { (self.0 >> 16) as u8 }
    pub const fn slot(&self)  -> u16 { self.0 as u16 }
}
