...
use crate::syscall;
use crate::syscalls::{SYS_THREAD_EXIT, SYS_THREAD_SPAWN};
...
unsafe fn run_fiber<F: FnOnce() -> u64>(closure_ptr: *mut u8, outcome: *const AtomicU64) -> ! {
    ...
    let result = f();
    unsafe {
        (*outcome).store(result, Ordering::Release);
    }
...
impl FabricFiber {
    const fn new() -> Self {
        Self {
            outcome: AtomicU64::new(OUTCOME_PENDING),
            tid: 0,
        }
    }
    ...
    pub fn spin_until_done(&self) -> u64 {
        loop {
            if let Some(v) = self.try_harvest() {
                return v;
            }
            core::hint::spin_loop();
        }
    ...
    F: FnOnce() -> u64 + Send,
{
    ...
    unsafe {
        core::ptr::write(clos_ptr, closure);
    }
